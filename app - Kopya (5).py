...
app = Flask(__name__)
CORS(app)
# YENİ EKLENEN SATIR: API yanıtlarının her zaman UTF-8 olmasını garantiler.
...
try:
    ...
    # Bold (kalın) versiyonu kaydet
    pdfmetrics.registerFont(TTFont('DejaVuSans-Bold', 'fonts/DejaVuSans.ttf'))
    # NOT: Eğer ayrı bir bold font dosyanız varsa, 'fonts/DejaVuSans-Bold.ttf' gibi yolunu belirtin.
    # Aksi takdirde, aynı dosyayı kullanmak ReportLab'in sentetik olarak kalın versiyonu oluşturmasını sağlar.
    print("Fontlar başarıyla kaydedildi.")
except Exception as e:
    print(f"Hata: Font dosyası yüklenemedi. 'fonts/DejaVuSans.ttf' dosyasının varlığından ve yolundan emin olun. Hata: {e}")
...
def generate_pdf(device, form_type):
    ...
    # Türkçe karakterler için fontu kullanın
    ...
    def draw_text_block(x, start_y, text, title):
        ...
        start_y -= 20
        # Metni ReportLab için güvenli hale getiriyoruz
        text_safe = str(text) if text is not None else ""
        lines = [text_safe[i:i+70] for i in range(0, len(text_safe), 70)] if text_safe else ["-"]
        for line in lines[:5]:
    ...
    if form_type in ["teslim_etme", "teklif"]:
        ...
        # Sayısal değerleri güvenli bir şekilde dönüştürün
        try:
            maliyet = float(device.maliyet) if device.maliyet is not None and device.maliyet != '' else 0.0
        except (ValueError, TypeError):
            maliyet = 0.0

        try:
            servis_ucreti = float(device.servis_ucreti) if device.servis_ucreti is not None and device.servis_ucreti != '' else 0.0
        except (ValueError, TypeError):
            servis_ucreti = 0.0

        p.drawString(100, y, f"Maliyet: {maliyet:.2f} TL")
        y -= 25
        p.drawString(100, y, f"Servis Ücreti: {servis_ucreti:.2f} TL")
        y -= 25
        p.drawString(100, y, f"Toplam: {maliyet + servis_ucreti:.2f} TL")
        y -= 30

    if form_type == "teklif":
        p.setFont("DejaVuSans", 12)
        teklif_durumu = str(device.teklif_durumu) if device.teklif_durumu is not None else 'Teklif Bekliyor'
        p.drawString(100, y, f"Teklif Durumu: {teklif_durumu}")
        y-= 30
